...
echo "########## [1/3] 后端 ##########"
if ! bash "${SCRIPT_DIR}/deploy-backend.sh" "${ENV_NAME}"; then
...
if [ "${SKIP_COLLAB}" != true ]; then
  echo ""
  echo "########## [2/3] 协同服务 ##########"
  if ! bash "${SCRIPT_DIR}/deploy-collab.sh" "${ENV_NAME}"; then
...
echo ""
echo "########## [3/3] 前端 ##########"
if ! bash "${SCRIPT_DIR}/deploy-frontend.sh" "${ENV_NAME}"; then
